...
같은 판정이 다른 조직에서 독립적으로 나왔다. 2026-02 개정 하네스 운영 지침은 "저장소 루트의
지침 파일은 도구별로 나누지 않고 하나를 정본으로 두고, 다른 도구용 파일은 정본을 가리키는
포인터로만 둔다"를 규칙으로 올리면서, 그 조항에 각주로 "포인터 방식은 하드 인바리언트가 소프트
지시로 약해지는 문제가 보고됐다. 재검토 대상"을 달아 두었다. 규칙과 그 규칙의 알려진 약점이
같은 문서 안에 공존하는 상태다.

### 분량 예산

같은 지침이 세 번째 조항으로 두는 것은 분량 통제다 — "지침 파일은 세션마다 전부 읽히므로 분량
예산을 둔다. 초과분은 별도 문서로 분리하고 포인터만 남긴다." 위의 § 지침은 강제가 아니다가
말하는 200줄 목표와 같은 동기이고, 포인터의 약점을 알면서도 포인터를 쓸 수밖에 없는 이유이기도
하다. 분량 예산과 하드 인바리언트의 강제력은 같은 축의 양 끝에서 서로를 당긴다.
