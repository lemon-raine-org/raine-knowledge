...
한 운영 지침 문서는 Rules/Skills 분리가 깨지는 방향을 양쪽으로 짚는다 — 규칙을 워크플로
안에 적어두면 그 워크플로를 부르지 않는 작업에서는 규칙이 사라지고, 반대로 워크플로 절차를
규칙에 적으면 모든 세션이 쓰지도 않을 절차를 매번 읽는다. 두 실패 모두 "항상 적용"과
"호출될 때만 적용"이라는 적용 시점의 차이를 무시한 결과다.

같은 문서는 변경 권한도 이 구분을 따라 가른다 — 규칙 변경은 최소 1명의 검토를 받고,
워크플로·전처리는 담당자가 단독으로 바꿀 수 있다. 규칙은 프로젝트 전체가 항상 전제로 삼는
것이라 검토 비용을 지불하고, 워크플로는 호출하는 사람이 곧 영향받는 사람이라 단독 변경이
합리적이라는 논리다.
