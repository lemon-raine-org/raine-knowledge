...
sources:
  ...
  - "raw/How Claude remembers your project - Claude Code Docs.md"  - "raw/AI 에이전트 하네스 운영 지침(2026-02 개정).md"
created: "2026-09-14"
updated: "2026-09-22"
---
...
### 세 번째 축과 변경 권한 — 다른 조직의 같은 분할

2026-02 개정 하네스 운영 지침은 같은 분할을 세 칸으로 넓히고, 각 칸에 **변경 권한**을 붙였다.
Cursor의 `rules`/`skills` 두 축에 입력 전처리가 별도 축으로 올라온 형태다.
...
"적용 시점" 칸이 분리의 근거를 그대로 드러낸다 — 규칙을 워크플로 안에 적어두면 그 워크플로를
부르지 않는 작업에서는 규칙이 사라지고, 반대로 워크플로 절차를 규칙에 적으면 모든 세션이 쓰지도
않을 절차를 읽는다. 이 조항은 2026-02 개정에서 추가됐고, 문서의 메모에 따르면 "규칙/워크플로
혼용으로 두 건의 사고가 있었다"는 것이 추가 사유다. (needs-update: 사고의 내용은 원문에 없다.)

변경 권한을 비대칭으로 둔 것도 이 축에서 나온다. 항상 적용되는 규칙만 합의를 거치고, 호출될
때만 적용되는 워크플로·전처리는 담당자 단독으로 바꾼다 — 잘못됐을 때 영향 범위가 다르기 때문이다.
